...
export default function Product({ params }) {
  ...
  const cartItem = cart
    ? cart.filter((item) => item["product"] == params.productid)[0]
    : null;

  const cartUnit = cartItem ? cartItem.units : 1;
  ...
  const refresh =()=> {
    fetch(`${process.env.NEXT_PUBLIC_URL}/products/${params.productid}`)
      .then((res) => res.json())
      .then((data) => {
        setProducts(data);
    ...
    fetch(`${process.env.NEXT_PUBLIC_URL}/cart`)
      .then((res) => res.json())
      .then((data) => {
        setCart(data);
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_URL}/similar`)
      .then((res) => res.json())
      .then((data) => {
        setSimilarList(data);
  ...
  const addToCart = () => {
    ...
    postData(`${process.env.NEXT_PUBLIC_URL}/add-to-cart`, newCartItem).then(
      (data) => {
        toast("Added to cart");
  ...
  const updateCart = (increment) => {
    ...
    const newCartItem = {
      _id: cartItem["_id"],
      units: increment ? units + 1 : units - 1,
    };

    postData(`${process.env.NEXT_PUBLIC_URL}/update-cart`, newCartItem).then(
      (newCartItem) => {
        toast("udpated to cart");
